    ...
    <div class="main">
      <div class="container">
        <div class="row">
          <div class="col-sm">
            <form id="workForm">
              <input type="hidden" id="workId" value="<?php echo "{$work['id']}"; ?>">
              <div class="form-group">
                <label for="introArea">簡介</label>
                <textarea class="form-control" id="introArea" rows="5"><?php echo "{$work['intro']}"; ?></textarea>
              </div>
              <div class="form-group">
                ...
                <div class="showImage" style="padding-top: 20px;">
                  <?php if(!is_null($work['image_path'])): ?>
              ...
              <div class="form-group">
                ...
                <div class="showVideo embed-responsive embed-responsive-16by9" style="padding-top: 20px;">
                  <?php if(!is_null($work['video_path'])): ?>
                    <iframe class='embed-responsive-item' src='<?php echo "../".$work['video_path']; ?>' allowfullscreen style='height: 300px;'></iframe>
                  <?php endif; ?>
                </div>
              ...
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="publish" id="inlineRadio1" value="1" <?php echo ($work['publish'] == 1)?'checked':''; ?>>
                <label class="form-check-label" for="inlineRadio1">公開</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="publish" id="inlineRadio2" value="0" <?php echo ($work['publish'] == 0)?'checked':''; ?>>
    ...
    <!-- 當文件準備好，要做的事情是 -->
      $(document).ready(function()
      {
        ...
        $("input.image").change(function()
        {
          ...
          $("div.showImage").html('<i class="fas fa-spinner fa-spin"></i>');
          $.ajax({
            type : 'POST',
        ...
        $("input.video").change(function()
        {
          ...
          form_data.append("file", file); // append(key, value)
          form_data.append("save_path", save_path);
          $("div.showVideo").html('<i class="fas fa-spinner fa-spin"></i>');
          $.ajax({
            type : 'POST',
            url : '../php/upload_file.php',
            data : form_data, // 上面的那個
        ...
        $("#workForm").submit(function(){
          if($("#introArea").val() == '')
          {
            ...
            $.ajax({// 要傳送的包裹
              type : "post",// 在後端用表單送出去
              url : "../php/update_work.php",// 要送給誰處理
              data :
              {
                'id' : $("#workId").val(),
