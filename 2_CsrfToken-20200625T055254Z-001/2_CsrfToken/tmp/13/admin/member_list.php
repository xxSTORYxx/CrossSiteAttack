                ...
                <?php if(!empty($members)): ?>
                  <?php foreach ($members as $member): ?>
                    <tr>
                      ...
                      <td>
                        <a href="member_edit.php?id=<?php echo "{$member['id']}"; ?>" class="btn btn-success">編輯</a>
                        <a href="javascript:void(0);" class="btn btn-danger del_member" data-id="<?php echo "{$member['id']}"; ?>">刪除</a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
      ...
      $(document).ready(function()
      {
        $("a.del_member").click(function(){
          ...
          if(c)
          {
            $.ajax({// 要傳送的包裹
              type : "post",// 在後端用表單送出去
              url : "../php/delete_member.php",// 要送給誰處理
              data :
              {
                'i' : $(this).attr("data-id")
