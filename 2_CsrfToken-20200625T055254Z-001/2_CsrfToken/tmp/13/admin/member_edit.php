<?php
  ...
  $member = get_edit_member($_GET['id']);
        ...
        <div class="row">
          <div class="col-sm">
            <form id="registerForm">
              <input type="hidden" id="memberId" value="<?php echo "{$member['id']}"; ?>">
              <div class="form-group row">
                <label for="name" class="col-sm-2 col-form-label">名稱</label>
                <div class="col-sm-10">
                  <input type="text" class="form-control" id="name" name="name" required value="<?php echo "{$member['name']}"; ?>">
                </div>
              </div>
              <div class="form-group row">
                <label for="username" class="col-sm-2 col-form-label" id="usernameLabel">帳號</label>
                <div class="col-sm-10">
                  <input type="text" class="form-control" id="username" name="username" required value="<?php echo "{$member['username']}"; ?>">
                </div>
              </div>
              <div class="form-group row">
                <label for="password" class="col-sm-2 col-form-label" id="passwordLabel">密碼</label>
                <div class="col-sm-10">
                  <input type="password" class="form-control" id="password" name="password" placeholder="(更改密碼)">
                </div>
              </div>
              <div class="form-group row">
    ...
    <!-- 當文件準備好，要做的事情是 -->
      $(document).ready(function()
      {
        ...
        $("#registerForm").submit(function(){
          ...
          $.ajax({// 要傳送的包裹
            type : "post",// 在後端用表單送出去
            url : "../php/update_member.php",// 要送給誰處理
            data :
            {
              'i' : $("#memberId").val(),
          ...
          {// 有正常接收訊息, 回傳的訊息叫作 data
            ...
            if(data == 'yes')
            {
              alert("更新成功，請按確認跳轉列表頁。");
              window.location.href = "member_list.php";
            }else
            {
              alert("更新失敗，請檢查網路連線狀態，若仍無法解決，請聯繫系統工程人員。");
            }
          }).fail(function(jqXHR, textStatus, errorThrown){
            // 失敗的時候
